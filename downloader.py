...
def fetch_playlist(playlist_url: str) -> dict:
    ...
    # Fast parallel preview URL & high-res 1000x1000 album cover art enrichment
    try:
        def _fetch_single_enrichment(t):
            ...
            try:
                r = requests.get(f"https://api.deezer.com/search?q={artist} {title}", headers=HEADERS, timeout=3).json()
                if r.get("data") and len(r["data"]) > 0:
                    item = r["data"][0]
                    if not prev_url:
                        prev_url = item.get("preview")
                    album = item.get("album", {})
                    high_cover = album.get("cover_xl") or album.get("cover_big") or album.get("cover_medium")
                    if high_cover:
                        cov_url = high_cover
            except Exception:
                pass
            ...
            if not cov_url:
                try:
                    r = requests.get(f"https://itunes.apple.com/search?term={artist} {title}&entity=song&limit=1", headers=HEADERS, timeout=3).json()
        ...
        with ThreadPoolExecutor(max_workers=12) as executor:
            enrichments = list(executor.map(_fetch_single_enrichment, result["tracks"]))

        for t, (p, c) in zip(result["tracks"], enrichments):
            if p:
                t["preview_url"] = p
            if c:
                t["cover_url"] = c
